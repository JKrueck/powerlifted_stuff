                                                ...
                                                Heuristic &heuristic)
{
    ...
    if (check_goal(task, generator, timer_start, task.initial_state, root_node, space, tracking_heuristic_cumulative)) return utils::ExitCode::SUCCESS;
    ...
    while (not queue.empty()) {
        ...
        if (check_goal(task, generator, timer_start, state, node, space,tracking_heuristic_cumulative)) return utils::ExitCode::SUCCESS;
        ...
        for (const LiftedOperatorId& op_id:applicable) {
            ...
            auto heur2 = Clock::now();
            int new_h = heuristic.compute_heuristic(s, task);
            tracking_heuristic_cumulative += Clock::now() - heur2;
            statistics.inc_evaluations();
